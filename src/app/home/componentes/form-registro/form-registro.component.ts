...
export class FormRegistroComponent implements OnInit {
  ...
  comprobarUsuario(formGroup: FormGroup) {
    ...
    this.usuariosService.getUsuarioConUsuario(usuario.value).subscribe((res:any) => {
  ...
  comprobarEmail(formGroup: FormGroup) {
    ...
    this.usuariosService.getUsuarioConEmail(email.value).subscribe((res:any) => {
      if (res.existeEmail)
        return email.setErrors({ existeEmail: true });
    });
  }

  //Crea un usuario en la tabla usuarios
  registrar(formGroup: FormGroup){
    const { nombre, apellidos, usuario, email, password } = this.registroForm.value;
    console.log(this.registroForm.value);
    if (nombre === "" || apellidos === "" || usuario === "" || email === "" || password === "")
      this.openSnackBar("Debe rellenar los campos obligatorios.");
      else {
        this.usuariosService.registrarUsuario(nombre, apellidos, usuario, email, password).subscribe((res) => {
